...
def create(patient_id=None):
    ...
    if request.method == 'POST':
        ...
        ai_result = predict_risk(content)
        handover.risk_level = ai_result.get("risk_level", "UNKNOWN")
        handover.risk_score = ai_result.get("risk_score", 0.0)
        ...
        AuditService.log_create(
            ...
            new_value={'patient': patient.name if patient else '?',
                       ...
                       'ai_risk': handover.risk_level,
                       'ai_score': handover.risk_score},
            description=f'인수인계 작성 (환자: {patient.name if patient else "?"}, 규칙: {assessment.risk_level}, AI: {handover.risk_level})'
        )
...
def edit(id):
    ...
    if request.method == 'POST':
        ...
        ai_result = predict_risk(handover.content)
        handover.risk_level = ai_result.get("risk_level", "UNKNOWN")
        handover.risk_score = ai_result.get("risk_score", 0.0)
        ...
        AuditService.log_update(
            ...
            description=f'인수인계 수정 → 규칙: {assessment.risk_level}, AI: {handover.risk_level}'
        )
